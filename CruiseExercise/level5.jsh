   ...
   for (Cruise cruise : cruises) {
       ...
       if (active.isEmpty() == true && expired.isEmpty() == true) {
           ...
           for (int i = 1; i < numLoaders + 1; i++) {
               ...
               if (i % 3 == 0) {
                   s = new Service(new RecycledLoader(i), cruise);
               }
               active = active.add(s);
               result = result.add(s);
}
       ...
       if (numLoaders > expiredCounter) {
           ...
           for (int i = 0; i < (numLoaders-expiredCounter); i++) {
               if ((expired.size() + active.size() + 1) % 3 == 0) {
                   expired = expired.add(new Service
                       (new RecycledLoader(expired.size() + active.size() + 1), cruise));
               } else {;
